...
// ============================================================
// ⚠️  PEGÁ TUS KEYS AQUÍ — SOLO 2 LÍNEAS
// ============================================================
const OPENAI_KEY = "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";   
// La Claude Key va en Vercel → Environment Variables → ANTHROPIC_API_KEY
// ============================================================
...
const MP_PLAN_URL = "https://www.mercadopago.com.ar/subscriptions";
const FREE_LIMIT  = 5;
const STORAGE_KEY = "animuspet_uses_v5";
const PETS_KEY    = "animuspet_pets_v5";
...
const DISEASES = {
  perro: {
    ...
    enojado:["Dolor crónico","Fobia situacional"],
    enfermo:["Distemper","Parvovirus","Artritis"],
...
async function initFirebase() {
  ...
  try {
    ...
    return { auth: fbAuth, GoogleAuthProvider, signInWithPopup, signOut };
  } catch (e) {
...
async function analyzeEmotion({ frame, audioText, captureMode, especie }) {
  ...
  const audioSection = audioText ? `\nSONIDO REAL detectado por Whisper: "${audioText}"\n` : "";
...
{"estado":"${estados}","confianza":85,"explicacion":"2-3 oraciones en español","senales_audio":["s1","s2"],"senales_visuales":["v1","v2"],"posible_enfermedad":false,"enfermedad_nota":""}`;
...
async function chatVet(messages, especie) {
  const system = `Sos el Dr. Paws, veterinario virtual de AnimusPet. Español rioplatense, cálido y profesional. Especialista en ${especie === "gato" ? "felinos" : "caninos"}. Máximo 3 párrafos. Nunca reemplazás consulta presencial. Terminás con consejo accionable.`;
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const saved = JSON.parse(localStorage.getItem(PETS_KEY) || "[]");
    setPets(saved);
    if (saved.length > 0) setActivePet(saved[0]);
  }, []);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.error(e);
      alert("Error al iniciar sesión. Verificá la configuración de Firebase.");
    }
  ...
  const startCamera = async () => {
    try {
      ...
      alert("Necesitamos acceso a tu cámara y micrófono.");
    }
  };
  ...
  const handleStartCapture = async (pet) => {
    if (usesLeft <= 0) { setScreen("paywall"); return; }
    const p = pet || activePet;
    if (!p) { setShowAddPet(true); return; }
    setActivePet(p);
    setEspecie(p.type);
    await startCamera();
    setScreen("capture");
  ...
  const handleVetSend = async () => {
    ...
    try {
      ...
      setVetMsgs([...newMsgs, { role: "assistant", content: "Error al conectar. Intentá de nuevo." }]);
    }
    setVetLoading(false);
  };
  ...
  const shareResult = (platform) => {
    if (!result) return;
    const m = (MOOD[result.petType] || MOOD.perro)[result.estado];
    const text = `${m?.emoji} ${result.petName} está ${m?.label} hoy!\nAnalizado con AnimusPet 🐾\nanimuspet.com`;
    const urls = {
      whatsapp: `https://wa.me/?text=${encodeURIComponent(text)}`,
      twitter:  `https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}`,
      facebook: `https://www.facebook.com/sharer/sharer.php?u=https://animuspet.com`,
    };
    window.open(urls[platform], "_blank");
  };

  const especieActual = result?.petType || especie || "perro";
  ...
  const accent   = (activePet?.type === "gato" || especie === "gato") ? "#9B7FD4" : "#D4AF37";
  ...
  const S = {
    ...
    mBtn:   (a) => ({ flex: 1, maxWidth: "140px", background: a ? "linear-gradient(135deg,#D4AF37,#B8963E)" : "#0F0F0F", color: a ? "#080808" : "#555", border: a ? "none" : "1px solid #222", borderRadius: "14px", padding: "13px 8px", cursor: "pointer", fontSize: "12px", fontWeight: a ? "700" : "400", textAlign: "center" }),
    ...
    chip:   (a) => ({ background: a ? "linear-gradient(135deg,#D4AF37,#B8963E)" : "#111", color: a ? "#080808" : "#888", border: a ? "none" : "1px solid #222", borderRadius: "50px", padding: "8px 16px", fontSize: "12px", fontWeight: a ? "700" : "400", cursor: "pointer", whiteSpace: "nowrap" }),
    share:  (c) => ({ background: c, color: "#fff", border: "none", borderRadius: "50px", padding: "10px 16px", fontSize: "12px", fontWeight: "700", cursor: "pointer", flex: 1 }),
  };
    ...
    <div style={S.app}>
      <nav style={S.nav}>
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          <button style={{ ...S.gBtn, fontSize: "16px", padding: "6px 10px" }} onClick={() => setScreen("history")}>📁</button>
          <button style={{ ...S.gBtn, background: "linear-gradient(135deg,#D4AF37,#B8963E)", color: "#080808", border: "none", fontWeight: "700" }} onClick={openVet}>🩺 Dr. Paws</button>
          {user
            ? <img src={user.photo} alt="" style={{ width: "30px", height: "30px", borderRadius: "50%", border: "2px solid #D4AF37", cursor: "pointer" }} onClick={handleLogout} />
      ...
      <div style={{ textAlign: "center", padding: "44px 20px 20px" }}>
        ...
        <h1 style={{ fontSize: "clamp(26px,7vw,44px)", fontWeight: "700", color: "#D4AF37", lineHeight: 1.05, marginBottom: "10px" }}>
        ...
        {pets.length > 0 && (
          <div style={{ marginBottom: "20px" }}>
            ...
            <div style={{ display: "flex", gap: "8px", justifyContent: "center", flexWrap: "wrap", padding: "0 16px" }}>
              {pets.map((p) => (
                <button key={p.id} style={S.chip(activePet?.id === p.id)} onClick={() => setActivePet(p)}>
                  {p.type === "gato" ? "🐱" : "🐶"} {p.name}
                </button>
              ))}
              <button style={{ ...S.chip(false), color: "#D4AF37", border: "1px solid #D4AF37" }} onClick={() => setShowAddPet(true)}>+ Agregar</button>
            </div>
          </div>
        )}
        ...
        {activePet ? (
          <div>
            <button style={S.pBtn} onClick={() => handleStartCapture(activePet)}>
          ...
          <button style={S.pBtn} onClick={() => setShowAddPet(true)}>+ Agregar mi primera mascota</button>
        )}
      </div>
            ...
            <div style={{ display: "flex", gap: "10px", marginBottom: "16px" }}>
              {[{ key: "perro", icon: "🐶", label: "Perro" }, { key: "gato", icon: "🐱", label: "Gato" }].map((t) => (
                <button key={t.key} onClick={() => setNewPetType(t.key)}
                  ...
                  <div style={{ fontSize: "28px", marginBottom: "4px" }}>{t.icon}</div>{t.label}
                </button>
              ))}
            </div>
            <input
      ...
      <div style={{ padding: "8px 12px 16px" }}>
        {[{ esp: "perro", color: "#D4AF37", label: "🐶 Estados — Perros" }, { esp: "gato", color: "#9B7FD4", label: "🐱 Estados — Gatos" }].map(({ esp, color, label }) => (
      ...
      {!user && (
        <div style={{ ...S.cGold, textAlign: "center" }}>
          ...
          <button style={{ ...S.pBtn, maxWidth: "220px" }} onClick={handleGoogleLogin}>{authLoading ? "Conectando..." : "🔑 Ingresar con Google"}</button>
        </div>
      )}
  ...
  if (screen === "capture") return (
    <div style={{ ...S.app, display: "flex", flexDirection: "column" }}>
      <nav style={S.nav}>
        <button style={{ ...S.gBtn, color: "#D4AF37", border: "none" }} onClick={() => { stopCamera(); setScreen("home"); }}>← Volver</button>
        <span style={S.logo}>{especie === "gato" ? "🐱" : "🐶"} {activePet?.name}</span>
      ...
      <div style={{ display: "flex", gap: "8px", margin: "8px 14px", justifyContent: "center" }}>
        <button style={S.mBtn(captureMode === "video")} onClick={() => setCaptureMode("video")}><div style={{ fontSize: "20px", marginBottom: "3px" }}>📹</div>Video</button>
        <button style={S.mBtn(captureMode === "audio")} onClick={() => setCaptureMode("audio")}><div style={{ fontSize: "20px", marginBottom: "3px" }}>🎙️</div>Audio</button>
      </div>
      <div style={{ margin: "0 14px", borderRadius: "18px", overflow: "hidden", border: `2px solid ${accent}`, background: "#000", aspectRatio: "4/3", position: "relative" }}>
        ...
        {recording && <div style={{ position: "absolute", top: "12px", right: "12px", display: "flex", alignItems: "center", gap: "5px", background: "#000a", borderRadius: "20px", padding: "4px 10px" }}>
        ...
        {recording && countdown > 0 && <div style={{ position: "absolute", bottom: "14px", left: "50%", transform: "translateX(-50%)", background: accent, color: "#080808", borderRadius: "50%", width: "44px", height: "44px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "20px", fontWeight: "900" }}>{countdown}</div>}
        {captureMode === "audio" && <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", background: "#000b" }}>
  ...
  if (screen === "analyzing") return (
    <div style={{ ...S.app, display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh" }}>
      <div style={{ textAlign: "center", padding: "40px 20px" }}>
        <div style={{ fontSize: "64px", marginBottom: "20px", display: "inline-block", animation: "spin 1.8s linear infinite" }}>{especie === "gato" ? "🐱" : "🐾"}</div>
        <h2 style={{ color: "#D4AF37", marginBottom: "6px", fontSize: "18px" }}>Analizando a {activePet?.name}...</h2>
        <p style={{ color: "#555", fontSize: "12px", maxWidth: "220px", margin: "0 auto 20px" }}>{analyzeStep}</p>
        <div style={{ display: "flex", flexDirection: "column", gap: "8px", maxWidth: "240px", margin: "0 auto" }}>
          {[{ icon: "🎙️", label: "Whisper", desc: "Transcribe audio real", done: analyzeStep.includes("Claude") }, { icon: "🧠", label: "Claude", desc: "Analiza postura + sonido", done: false }].map((step, i) => (
  ...
  if (screen === "result" && result && moodInfo) return (
    <div style={S.app}>
      <nav style={S.nav}>
        <button style={{ ...S.gBtn, color: "#D4AF37", border: "none" }} onClick={() => setScreen("home")}>← Inicio</button>
      ...
      <div style={{ background: moodInfo.grad, borderRadius: "22px", padding: "30px", margin: "12px", textAlign: "center" }}>
        ...
        <div style={{ background: "#00000018", borderRadius: "12px", padding: "4px 14px", display: "inline-block", marginBottom: "12px" }}>
          <span style={{ color: "#080808", fontSize: "11px", fontWeight: "700" }}>{result.petType === "gato" ? "🐱 Gato" : "🐶 Perro"} · {result.confianza}% confianza</span>
        </div>
      ...
      <div style={S.card}>
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          <button style={S.share("#25D366")} onClick={() => shareResult("whatsapp")}>💬 WhatsApp</button>
          <button style={S.share("#1DA1F2")} onClick={() => shareResult("twitter")}>🐦 Twitter</button>
          <button style={S.share("#1877F2")} onClick={() => shareResult("facebook")}>👥 Facebook</button>
        </div>
      </div>

      {result.audioTranscription && (
        <div style={{ ...S.card, border: "1px solid #1E3A1E" }}>
          <div style={{ color: "#4CAF50", fontSize: "10px", fontWeight: "700", marginBottom: "6px", textTransform: "uppercase" }}>🎙️ Whisper escuchó</div>
          <p style={{ color: "#777", fontSize: "12px", fontStyle: "italic", margin: 0 }}>"{result.audioTranscription}"</p>
        </div>
      )}
      ...
      {(result.senales_audio?.length > 0 || result.senales_visuales?.length > 0) && (
        <div style={S.card}>
          {result.senales_audio?.length > 0 && <>
            <h3 style={{ color: "#4CAF50", fontSize: "11px", marginBottom: "8px", textTransform: "uppercase" }}>🎙️ Señales de Audio</h3>
          ...
          {result.senales_visuales?.length > 0 && <>
            <h3 style={{ color: "#D4AF37", fontSize: "11px", margin: "12px 0 8px", textTransform: "uppercase" }}>📸 Señales Visuales</h3>
      ...
      <div style={S.cGold}>
        <h3 style={{ color: "#D4AF37", fontSize: "11px", marginBottom: "8px", textTransform: "uppercase" }}>💡 Consejo</h3>
        <p style={{ color: "#bbb", fontSize: "13px", lineHeight: 1.6, margin: 0 }}>{moodInfo.tip}</p>
      </div>

      {diseases.length > 0 && (
        <div style={{ ...S.card, border: "1px solid #3A1E1E" }}>
          <h3 style={{ color: "#E74C3C", fontSize: "11px", marginBottom: "10px", textTransform: "uppercase" }}>🏥 Condiciones a descartar</h3>
      ...
      <div style={{ padding: "12px", display: "flex", flexDirection: "column", gap: "10px", paddingBottom: "36px" }}>
        <button style={{ ...S.pBtn, fontSize: "16px", padding: "16px" }} onClick={openVet}>🩺 Consultar con Dr. Paws</button>
    ...
    <div style={S.app}>
      ...
      {pets.length === 0 ? (
        ...
        <div style={{ padding: "14px" }}>
          {pets.map((pet) => {
            const pm   = MOOD[pet.type] || MOOD.perro;
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    ...
                    <div>
                      ...
                      <div style={{ color: "#555", fontSize: "11px" }}>{hist.length} análisis</div>
                    </div>
                  </div>
                  <button style={{ ...S.pBtn, width: "auto", padding: "8px 16px", fontSize: "12px" }} onClick={() => handleStartCapture(pet)}>+ Analizar</button>
                </div>
                {hist.length > 0 && (
                  <div style={{ display: "flex", gap: "8px", overflowX: "auto", paddingBottom: "8px", marginBottom: "10px" }}>
                    {Object.entries(pm).map(([key, m]) => {
  ...
  if (screen === "vet") return (
    <div style={{ ...S.app, display: "flex", flexDirection: "column", height: "100vh" }}>
      <nav style={S.nav}>
        <button style={{ ...S.gBtn, color: "#D4AF37", border: "none" }} onClick={() => setScreen(result ? "result" : "home")}>← Volver</button>
  ...
  if (screen === "paywall") return (
    <div style={S.app}>
      <nav style={S.nav}>
        <button style={{ ...S.gBtn, color: "#D4AF37", border: "none" }} onClick={() => setScreen("home")}>← Volver</button>
        <span style={S.logo}>🐾 ANIMUSPET</span>
        <span style={{ width: "60px" }} />
      </nav>
      ...
      <div style={{ background: "linear-gradient(135deg,#150F00,#0A0800)", border: "2px solid #D4AF37", borderRadius: "22px", padding: "28px", margin: "14px", textAlign: "center" }}>
        ...
        {["✅ Análisis ilimitados", "✅ Múltiples mascotas", "✅ Dr. Paws sin límites", "✅ Audio real via Whisper", "✅ Historial completo"].map((f) => (
          <div key={f} style={{ color: "#bbb", fontSize: "13px", marginBottom: "7px", textAlign: "left" }}>{f}</div>
        ))}
        <button style={{ ...S.pBtn, marginTop: "20px" }} onClick={() => window.open(MP_PLAN_URL, "_blank")}>💳 Activar con MercadoPago</button>
      </div>
